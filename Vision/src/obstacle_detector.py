...
class ObstacleDetector:
    ...
    def detect(self, color_frame, depth_frame, depth_scale=0.001, danger_threshold=1.0, warning_threshold=3.0):
        """
        ...
        # Pembagian 3 Zona
        zone_width = width // 3
        cv2.line(annotated_frame, (zone_width, 0), (zone_width, height), (255, 255, 255), 2)
        cv2.line(annotated_frame, (zone_width * 2, 0), (zone_width * 2, height), (255, 255, 255), 2)
        ...
        for contour in contours:
            ...
            if valid_depth.size > 0:
                ...
                label = f"{zone_str.upper()}: {distance:.2f} m"
                
                # Gambar Bounding Box dan Teks
                cv2.rectangle(annotated_frame, (x, y), (x + w, y + h), color, 2)
                cv2.putText(annotated_frame, label, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
        ...
        cv2.putText(annotated_frame, f"STATUS: {global_status}", (30, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, status_color, 2)
